...
system.waitTicks(0).then(() => {
    ...
    router.registerListener("leafnet:req", async (payload) => {
        try {
            ...
            if (data.body)
                req.body = JSON.stringify(data.body);
            else req.body = "{}";
            // @ts-ignore
            req.method = data.method
            req.headers = [];
            if (data.headers) {
                for (const header2 in data.headers) {
                    let header = data.headers[header2];
                    req.headers.push(new net.HttpHeader(header.key, header.value));
                }
            }
            req.headers.push(new net.HttpHeader("Content-Type", "application/json"));
